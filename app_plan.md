...
**Status 2026-07-28 — accounts decision (app_issues.md Section 0): Option A, ship v1 with accounts parked.** The explore and profile tabs stay hidden (`href: null`), so no user or reviewer can reach sign-in. Consequences for this plan: **1.3 (magic-link callback) is moot for v1** — the handler ships in the binary but is unreachable; the end-to-end device test moves to post-launch, when accounts unpark. **4.3 review notes must describe an auth-less app**: no login, no reviewer test account needed; trips and carts live on-device; booking buttons are hand-off links out to travel providers (travel is exempt from IAP), not in-app purchases. Do not declare account creation or email collection in the ASC privacy labels for v1.
