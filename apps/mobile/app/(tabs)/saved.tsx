...
export default function SavedScreen() {
  ...
  return (
    ...
    >
      <View className="bg-card rounded-2xl p-5 mb-4 border border-line">
      ...
      {items.map((i) => (
        <View
          key={i.id}
          className="bg-card rounded-2xl p-4 mb-3 border border-line"
        >
          <Text className="text-ink-faint text-[11px] uppercase tracking-wider">
            {i.type}
          </Text>
          <Text className="text-ink text-[16px] font-semibold mt-1">
            {i.title}
          </Text>
          {i.subtitle ? (
          ...
          <Text className="text-ink text-[15px] font-semibold mt-2">
