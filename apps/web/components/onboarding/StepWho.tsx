...
export default function StepWho({ prefs, update, onNext, onBack }: Props) {
  return (
    <div className="flex flex-col flex-1">
      ...
      <div className="grid grid-cols-2 gap-3 mb-8">
        {types.map((t) => (
          <button
            ...
            className={`flex flex-col items-center justify-center py-6 rounded-[8px] border transition-all ${
      ...
      {/* Traveler count */}
      <div className="bg-gray-light rounded-[8px] px-5 py-4 flex items-center justify-between mb-10">
        <span className="text-gray-dark font-medium">Travelers</span>
        <div className="flex items-center gap-4">
          <button
            onClick={() => update({ travelers: Math.max(1, prefs.travelers - 1) })}
            className="w-9 h-9 rounded-full border border-black/10 bg-white text-gray-dark font-semibold text-lg flex items-center justify-center hover:border-accent/30 transition"
          >
            −
          </button>
