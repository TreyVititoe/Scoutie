      ...
      <header className="fixed top-0 left-0 right-0 z-20 nav-glass">
        <div className="max-w-content mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <button
            ...
            className="text-sm font-semibold text-ink-soft hover:text-ink transition-colors flex items-center gap-1.5"
          >
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-[21px] font-semibold text-ink">Your Trips</h2>
              </div>
              {trips.length >= 2 && (
                <button
                  onClick={() => {
            ...
            {compareMode && compareIds.length > 0 && (
              ...
              >
                <p className="text-sm text-ink-soft">
                  ...
                  <span className="text-ink-faint ml-1">(select 2-3)</span>
                </p>
                <button
            ...
            {trips.length === 0 ? (
              <div className="card-base p-10 text-center">
                <span className="material-symbols-outlined text-ink-faint text-3xl mb-3 block">explore</span>
                <p className="font-semibold text-ink mb-1">No trips yet</p>
                    ...
                    >
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <h3 className="font-semibold text-ink pr-8">
                            {trip.title || trip.destination}
                          </h3>
                          {trip.tier && (
                            <span className="bg-[#DBEAFE] text-accent rounded-pill px-2 py-0.5 text-[10px] font-semibold flex-shrink-0">
                              {trip.tier.charAt(0).toUpperCase() + trip.tier.slice(1)}
                            </span>
                          )}
                        </div>
                        {trip.title && trip.destination && (
                          <p className="text-sm text-ink-soft mb-2">{trip.destination}</p>
                        )}
