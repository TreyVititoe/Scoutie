/*
 * Honest trip economics from a saved cart. Prices in the cart are what
 * the whole group pays (flights are roundtrip totals, events carry all
 * tickets), so the math here works on group totals and derives per-person
 * and per-day views from them. The one estimate is the daily food and
 * getting-around allowance, and it is always labeled as an estimate.
 */

import type { SavedTrip } from "@/lib/stores/savedTripsStore";
...
export function tripEconomics(
  ...
  const travelers = trip.travelers ?? fallbackTravelers;
  ...
  /* Flight length: the search result rides along in meta; older items
   * fall back to the "5h 20m out ..." subtitle. */
