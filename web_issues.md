...
_**Status 07-30:** all six launch gates (B1-B6) closed, plus H1-H3, H6, H7, H9, H10, H13, M14, M15. See the per-item notes for the two that were fixed differently than suggested. Remaining: H4, H5 (partial), H8, H11, H12, H14, and the medium/low tails._
...
**API & cost**
- [ ] **H5. Rate limiter is per-instance in-memory, and 8 routes have none at all.** _PARTIAL 07-30: rateLimit added to trips/refine, trips/share, trips/save, affiliate/click. Still uncovered: trips/shared, trips/upvote, trips/community, photo. The shared-store swap (Upstash/KV) is untouched._ Limits multiply with Vercel concurrency and reset on cold start; uncovered: trips/refine, trips/share, trips/shared, trips/save, trips/upvote, trips/community, affiliate/click, photo. Fix: shared store (Upstash/KV or WAF rules) + cover the stragglers. (`lib/apiGuard.ts:10-12`)
